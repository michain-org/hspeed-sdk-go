...
func init() {
	...
	//RegisterHash(BLAKE2s_256, blaske2s.BLAKE2b_256)
	//RegisterHash(BLAKE2s_256, blaske2s.BLAKE2b_384)
	//RegisterHash(BLAKE2s_256, blaske2s.BLAKE2b_512)
...
func getSignatureAlgorithmFromAI(ai pkix.AlgorithmIdentifier) SignatureAlgorithm {
	...
	case params.Hash.Algorithm.Equal(oidHashSM3):
		//TODO
		return SM2WithSM3
	}
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			...
			case 31:
				...
				for _, dp := range cdp {
					...
					if len(dp.DistributionPoint.FullName[0].Bytes) == 0 {
						continue
					}

					var n asn1.RawValue
					if _, err := asn1.Unmarshal(dp.DistributionPoint.FullName[0].Bytes, &n); err != nil {
						return nil, err
					}
...
func ParseCertificates(asn1Data []byte) ([]*Certificate, error) {
	...
	x509Certs, err := x509.ParseCertificates(asn1Data)
		...
		for _, name := range template.CRLDistributionPoints {
			...
			dp := distributionPoint{
				DistributionPoint: distributionPointName{
					FullName: []asn1.RawValue{asn1.RawValue{Tag: 0, Class: 2, IsCompound: true, Bytes: rawFullName}},
				},
			}
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub, priv interface{}) (cert []byte, err error) {
	if _, ok := pub.(*ecdsa.PublicKey); ok {
		x509Template := toX509Cert(template)
		x509parent := toX509Cert(parent)
		return x509.CreateCertificate(rand, x509Template, x509parent, pub, priv)
	}
	if _, ok := pub.(*sm2.PublicKey); ok {
		//var sm2Key *sm2.PrivateKey
		return createSM2Certificate(rand, template, parent, pub, priv)
	}
	return nil, errors.New("CreateCertificate: pub type must be ecdsa or sm2")
}

func createSM2Certificate(rand io.Reader, template, parent *Certificate, pub, priv interface{}) (cert []byte, err error) {
...
func toX509Cert(cert *Certificate) *x509.Certificate {
	toCert := &x509.Certificate{
		Raw:                         cert.Raw,
		...
		PermittedEmailAddresses:     cert.PermittedDNSDomains,
	...
	for _, ku := range cert.ExtKeyUsage {
		extKeyUsage = append(extKeyUsage, x509.ExtKeyUsage(ku))
	}
	toCert.ExtKeyUsage = extKeyUsage
	return toCert
}

func toCert(cert *x509.Certificate) *Certificate {
	toCert := &Certificate{
		Raw:                         cert.Raw,
		RawTBSCertificate:           cert.RawTBSCertificate,
		RawSubjectPublicKeyInfo:     cert.RawSubjectPublicKeyInfo,
	...
	for _, ku := range cert.ExtKeyUsage {
		extKeyUsage = append(extKeyUsage, ExtKeyUsage(ku))
	}
	toCert.ExtKeyUsage = extKeyUsage
	return toCert
}
