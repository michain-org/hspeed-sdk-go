...
func verifyHandshakeSignature(sigType uint8, pubkey crypto.PublicKey, hashFunc crypto.Hash, signed, sig []byte) error {
	...
	case signatureECDSA:
		...
		if !ok {
			if pubKey, ok := pubkey.(*sm2.PublicKey); ok {
				...
				if !sm2.Sm2Verify(pubKey, signed, nil, sm2Sig.R, sm2Sig.S) {
					return errors.New("tls: ECDSA verification failure")
				}
				return nil
			}
			return errors.New("tls: ECDSA signing requires a ECDSA public key or a SM2 public key")
		}
...
func signatureSchemesForCertificate(version uint16, cert *Certificate) []SignatureScheme {
	...
	case *sm2.PublicKey:
		if version != VersionTLS13 {
			// In TLS 1.2 and earlier, SM2 algorithms are not
			// constrained to a single curve.
			return []SignatureScheme{
				ECDSAWithP256AndSHA256,
			}
