...
func createCollectionConfigPackage(ccFileByte []byte) (*pb.CollectionConfigPackage, error) {
	var ccp *pb.CollectionConfigPackage
	if len(ccFileByte) > 0 {
		var err error
		ccp, _, err = getCollectionConfigFromBytes(ccFileByte)
		if err != nil {
			return nil, errors.WithMessagef(err, "invalid collection configuration in filebyte %s", ccFileByte)
		}
	}
	return ccp, nil
}
